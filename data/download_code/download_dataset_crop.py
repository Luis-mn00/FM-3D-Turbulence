...
dt = 0.1
time_steps = np.arange(8.0, 10.0, dt)  # From 0 to 10 inclusive, step 0.1
...
total_size = 1024
delta = 2 * np.pi / total_size
x = np.linspace(0, delta * grid_size, grid_size, endpoint=False)
y = np.linspace(0, delta * grid_size, grid_size, endpoint=False)
z = np.linspace(0, delta * grid_size, grid_size, endpoint=False)
...
output_file = f"data_crop_5_{dt}_{grid_size}.pt"
